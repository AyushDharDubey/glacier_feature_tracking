...
**Module** `glacier_tlc/track_stage.py`, algorithm in `glacier_tlc/tracking.py`
...
**Module** `glacier_tlc/lake.py` · **writes** `lake_index.csv`

The paper documents the June 2024 evolution of the proglacial lake — ice-covered, then
clear, then turbid and sediment-laden, then clear again — by visual inspection of the
TLC and PlanetScope images [4.2]. This stage makes the same transitions quantitative by
summarising a configurable lake rectangle per image: mean brightness and saturation,
circular-mean hue, the red-to-blue ratio (a turbidity proxy, above 1 for sediment-laden
water and below 1 for clear blue-green water) and an ice fraction (bright, unsaturated
pixels).

This is an **addition**, not a reproduction: the paper reports no such index. It is
optional and is skipped when no `lake_rect` is configured.
