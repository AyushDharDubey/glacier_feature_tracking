...
STAGES = ["inventory", "quality", "extract", "track", "velocity", "aggregate", "compare", "lake", "plots", "report"]
...
def main(argv=None):
    ...
    for st in stages:
        if st == "lake" and a.stage == "all" and (a.skip_lake or not any(g.lake_rect for g in cfg.groups.values())):
            continue
        t0 = time.time()
        log.info("=== stage %s ===", st)
        mod = __import__(f"glacier_tlc.{ {'track': 'track_stage'}.get(st, st) }", fromlist=["run"])
