...
class Group:
    ...
    lake_rect: tuple[int, int, int, int] | None = None
...
def load_config(path: str | Path) -> Config:
    ...
    for gname, g in raw["groups"].items():
        ...
        lake = g.get("lake_rect")
        groups[gname] = Group(
            gname, _date(g["start"]), _date(g["end"]), rois, g.get("reference_image"),
            tuple(int(v) for v in lake) if lake else None, g.get("flow_sign_x", raw.get("velocity", {}).get("flow_sign_x", "auto")),
        )
